...
### scratch-notes.md - Temporary Workspace Notes

**When to use:** During active implementation for disposable notes

**Purpose:** Temporary notes, debug logs, and findings during active work

**Created by:** Agent during implementation (as needed)

**Location:** `scratch/` subfolder (NEVER in spec root)

**Copy command:**
```bash
mkdir -p specs/###-name/scratch
# Create as needed during work - no formal template
```
...
**Adaptation tips:**
- Use freely during work - this is disposable
- No formal structure required
- **MUST be in `scratch/` folder** - never pollute spec root
- Clean up when work is complete
- Move valuable findings to `memory/` before cleanup

**Size:** Variable (disposable)

**Cleanup rule:** Delete or archive `scratch/` contents when task completes

---

### memory-context.md - Context Preservation (Auto-Generated)

**When to use:** Preserve important context for future sessions

**Purpose:** Semantic memory-indexed context with ANCHOR format for retrieval

**Created by:** `generate-context.js` script (NEVER manually created)

**Location:** `memory/` subfolder

**Creation command:**
```bash
node .opencode/scripts/generate-context.js specs/###-name
```
...
**Adaptation tips:**
- Let the script generate structure
- Review generated content for accuracy
- Add additional context via script prompts
- Don't edit ANCHOR markers manually

**Size:** Variable (depends on session complexity)
