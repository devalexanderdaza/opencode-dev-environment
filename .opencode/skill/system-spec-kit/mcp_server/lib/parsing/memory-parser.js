...
function extract_trigger_phrases(content) {
  ...
  // Method 1a: Check YAML frontmatter inline format (support both snake_case and camelCase)
  // Pattern: trigger_phrases: [...] OR triggerPhrases: [...]
  const inline_match = content.match(/(?:trigger_phrases|triggerPhrases):\s*\[([^\]]+)\]/i);
  if (inline_match) {
...
function extract_context_type(content) {
  ...
  // Check YAML metadata block (support both snake_case and camelCase)
  // Match: context_type: value OR contextType: value
  const yaml_match = content.match(/(?:context_type|contextType):\s*["']?(\w+)["']?/i);
  if (yaml_match) {
...
function extract_importance_tier(content) {
  ...
  // Check YAML metadata block (support both snake_case and camelCase)
  // Match: importance_tier: value OR importanceTier: value
  const yaml_match = content.match(/(?:importance_tier|importanceTier):\s*["']?(\w+)["']?/i);
  if (yaml_match) {
